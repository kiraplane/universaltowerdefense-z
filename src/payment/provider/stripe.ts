...
export class StripeProvider implements PaymentProvider {
  ...
  public async createCreditCheckout(
    ...
    try {
      ...
      const checkoutParams: Stripe.Checkout.SessionCreateParams = {
        ...
        cancel_url: cancelUrl ?? '',
        metadata: customMetadata,
        allow_promotion_codes: allowPromotionCode ?? false,
      };
  ...
  private async processSubscriptionPurchase(
    ...
    try {
      const { trackServerEvent } = await import('@/lib/gtm');
      ...
      if (plan && price) {
        await trackServerEvent(
          'purchase',
          {
            ...
            value: price.amount / 100, // Convert cents to dollars
  ...
  private async processCreditPurchase(
    ...
    const expireDays = metadata.expireDays
      ? Number.parseInt(metadata.expireDays, 10)
      : undefined;
    ...
    await addCredits({
      ...
      description: `+${credits} credits for package ${packageId} ($${amountPaid.toLocaleString()})`,
    ...
    // Track credit purchase event (server-side)
    try {
      const { trackServerEvent } = await import('@/lib/gtm');
      ...
      await trackServerEvent(
        'purchase_credits',
        {
          ...
          credits_amount: Number.parseInt(credits),
          user_id: paymentRecord.userId,
        },
        paymentRecord.userId
      );
    } catch (error) {
      console.error('Failed to track credit purchase event:', error);
    }
