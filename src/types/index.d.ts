...
export interface StorageConfig {
  ...
  provider: 's3' | 's3-aws-sdk' | 's3mini' | 'aws-sdk'; // The storage provider (s3-aws-sdk recommended for better compatibility)
}
...
export type MenuItem = {
  ...
  linkTitle?: string;                 // Optional title attribute for the link
  ...
  isImage?: boolean;                  // Whether to display as an image instead of text
  imageSrc?: string;                  // The image source URL (required if isImage is true)
...
export type NestedMenuItem = MenuItem & {
  layout?: 'list' | 'inline';         // The layout style for nested items
