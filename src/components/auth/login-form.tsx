...
export const LoginForm = ({
  ...
  // Priority: propCallbackUrl > paramCallbackUrl > current pathname (unless on auth pages)
  // If user is on /auth/login page, redirect to homepage after login
  const isAuthPage =
    pathname?.startsWith('/auth') || pathname?.includes('/auth');
  const fallbackUrl = isAuthPage ? '/' : pathname || '/';
  const callbackUrl = propCallbackUrl || paramCallbackUrl || fallbackUrl;
    ...
    await authClient.signIn.email(
      ...
      },
      {
        ...
        onSuccess: (ctx) => {
          ...
          trackAuth('login', 'email', ctx.data?.user?.id);
