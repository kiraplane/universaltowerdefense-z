...
import { trackBeginCheckout, trackEvent } from '@/lib/gtm';
...
interface CheckoutButtonProps {
  ...
  planId: string;
  priceId: string;
  amount?: number; // Price amount in cents for tracking
  currency?: string; // Currency for tracking
  ...
  const handleClick = async () => {
    try {
      ...
      if (result?.data?.success && result.data.data?.url) {
        ...
        if (amount) {
          trackBeginCheckout(planId, amount, currency);
        }
