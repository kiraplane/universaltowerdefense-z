...
import {
  useAdminUserCreditBalance,
  useGrantUserCredits,
} from '@/hooks/use-admin-credits';
...
export function UserDetailViewer({ user }: UserDetailViewerProps) {
  ...
  const grantUserCreditsMutation = useGrantUserCredits();
  const { data: currentCredits, isLoading: isLoadingCredits } =
    useAdminUserCreditBalance(user.id, open);
  ...
  const handleGrantCredits = async () => {
    ...
    const parsedAmount = Number.parseInt(grantAmount, 10);
    if (!Number.isFinite(parsedAmount) || parsedAmount <= 0) {
      setError(t('grantCredits.invalidAmount'));
      return;
    }

    if (!grantReason.trim()) {
      setError(t('grantCredits.invalidReason'));
      return;
    }
    ...
    try {
      await grantUserCreditsMutation.mutateAsync({
        userId: user.id,
        amount: parsedAmount,
        reason: grantReason.trim(),
      });

      toast.success(t('grantCredits.success'));
      setGrantReason('');
    } catch (err) {
      const error = err as Error;
      console.error('Failed to grant credits:', error);
      setError(error.message || t('grantCredits.error'));
      toast.error(error.message || t('grantCredits.error'));
    }
  };
          ...
          <div className="grid gap-4">
            <div className="flex items-center justify-between gap-2">
              <span className="text-muted-foreground">
                {t('grantCredits.currentBalance')}
              </span>
              <Badge variant="outline" className="px-2 py-1">
                <CoinsIcon className="size-4" />
                {isLoadingCredits
                  ? t('loading')
                  : `${currentCredits || 0} ${t('grantCredits.balanceUnit')}`}
              </Badge>
            </div>
            ...
            >
              <div className="grid gap-2">
                <Label htmlFor={`grant-amount-${user.id}`}>
                  {t('grantCredits.amount')}
                </Label>
                <Input
              ...
              <div className="grid gap-2">
                <Label htmlFor={`grant-reason-${user.id}`}>
                  {t('grantCredits.reason')}
                </Label>
                <Textarea
                  id={`grant-reason-${user.id}`}
                  value={grantReason}
                  onChange={(event) => setGrantReason(event.target.value)}
                  placeholder={t('grantCredits.reasonPlaceholder')}
                  required
                />
              </div>
              <Button
                type="submit"
                disabled={
                  grantUserCreditsMutation.isPending ||
                  !grantReason.trim() ||
                  isDemo
                }
