...
import { resolveCreditPackageForCheckout } from '@/credits/admin-test-package';
...
export const createCreditCheckoutSession = userActionClient
  ...
  .action(async ({ parsedInput, ctx }) => {
    ...
    try {
      ...
      const creditPackage = await resolveCreditPackageForCheckout({
        packageId,
        currentUser,
      });
      if (!creditPackage) {
      ...
      if (creditPackage.price.priceId !== priceId) {
        return {
          success: false,
          error: 'Credit package price mismatch',
        };
      }
      ...
      const customMetadata: Record<string, string> = {
        ...
        expireDays: (creditPackage.expireDays || 0).toString(),
      ...
      const params: CreateCreditCheckoutParams = {
        ...
        allowPromotionCode: creditPackage.price.allowPromotionCode ?? false,
